...
db.knex.schema.hasTable('users').then(function(exists) {
  if (!exists) {
    db.knex.schema.createTable('users', function (users) {
      ...
      users.string('email_id', 255) // listed here as unique as a backup in case our unique check fails
...
db.knex.schema.hasTable('performances').then(function(exists) {
  if (!exists) {
    db.knex.schema.createTable('performances', function (performances) {
      ...
      // performances.integer('artist_id').unsigned().references('id').inTable('artists'); // artist
      // performances.integer('tag_id').unsigned().references('id').inTable('tags'); // tag
    })
    .then(function (table) {
      console.log('Created Table performances', table);
    });
  }
});
